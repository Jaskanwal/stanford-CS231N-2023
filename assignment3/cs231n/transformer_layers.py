...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        exponents = (
          10000**(-torch.arange(0, embed_dim, 2).view(1, -1) / embed_dim) * 
          torch.arange(0, max_len, 1).view(-1, 1)
        )

        even_idx = torch.arange(0, embed_dim, 2)
        pe[0,:,even_idx] = torch.sin(exponents)
        pe[0,:,even_idx+1] = torch.cos(exponents)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # Linear pass of Query, Key and Value and get them in correct shaps
        query = self.query(query)
        query = query.view(N, S, self.n_head, self.head_dim)
        query = torch.permute(query, (0, 2, 1, 3)) # (N, H, S, E/H)

        key = self.key(key)
        key = key.view(N, T, self.n_head, self.head_dim)
        key = torch.permute(key, (0, 2, 3, 1)) # (N, H, E/H, T)

        value = self.value(value)
        value = value.view(N, T, self.n_head, self.head_dim)
        value = torch.permute(value, (0, 2, 1, 3)) # (N, H, T, E/H)

        # Calculate attention weights
        score = torch.matmul(query, key)/math.sqrt(self.head_dim) # (N, H, S, T)

        if attn_mask is not None:
          score = score.masked_fill(attn_mask.view(1,1,*attn_mask.size())==0, float('-inf'))

        attention_weights = F.softmax(score , dim=-1) # (N, H, S, T)
        
        attention_weights = self.attn_drop(attention_weights)
        
        # Weight values based on attention weights and get output
