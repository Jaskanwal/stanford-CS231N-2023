...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    ht = h0
    for t in range(T):
      ht, cache[1][t] = rnn_step_forward(x[:,t,:].squeeze(), ht, Wx, Wh, b)
      h[:,t,:] = ht
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
      dx_t, dprev_h_t, dWx_t, dWh_t, db_t = (
        rnn_step_backward(dh[:,t,:] + dprev_h_t, cache[1][t])
      )
      dx[:,t,:] = dx_t
      dWx += dWx_t
      dWh += dWh_t
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (a, i, f, o, g, x, prev_h, prev_c, Wx, Wh, b, next_h, next_c)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    a, i, f, o, g, x, prev_h, prev_c, Wx, Wh, b, next_h, next_c = cache

    # Calculate gradients of terms involved in next_h = o * np.tanh(next_c)
    do = dnext_h * np.tanh(next_c) # (N, H)
    dnext_c += dnext_h * o * (1-np.tanh(next_c) **2) # (N, H)
    
    # Calculate gradients of terms involved in next_c = f * prev_c + i * g
    df = dnext_c * prev_c     # (N, H)
    dprev_c = dnext_c * f     # (N, H)
    di = dnext_c * g          # (N, H)
    dg = dnext_c * i          # (N, H)
    
    # Calculate gradients of activation a
    da = np.hstack([di * i * (1-i), # Gradient of sigmoid function 
                    df * f * (1-f), # Gradient of sigmoid function
                    do * o * (1-o), # Gradient of sigmoid function
                    dg * (1-g**2)], # Gradient of tanh function 
                    )  # (N, 4H)

    # Calculate gradients of terms involved in a = x @ Wx + prev_h @ Wh + b
    dx = da @ Wx.T   # (N, D)
    dWx = x.T @ da   # (D, 4H)

    dprev_h = da @ Wh.T # (N, D)
    dWh = prev_h.T @ da   # (D, 4H)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    ct = np.zeros_like(h0)
    for t in range(T):
      ht, ct, cache[1][t] = lstm_step_forward(x[:,t,:].squeeze(), ht, ct, Wx, Wh, b)
      h[:,t,:] = ht
...
def lstm_backward(dh, cache):
    ...
    dprev_c_t = np.zeros((N, H)) # c is not getting into loss function computation directly
    for t in reversed(range(T)):
      dx_t, dprev_h_t, dprev_c_t, dWx_t, dWh_t, db_t = (
        lstm_step_backward(dh[:,t,:] + dprev_h_t, dprev_c_t, cache[1][t])
      )
      dx[:,t,:] = dx_t
      dWx += dWx_t
      dWh += dWh_t
