...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        #1a Embed captions and add positional encoding
        captions = self.positional_encoding(self.embedding(captions))
        #1b Project images to the same dimension as captions
        features = self.visual_projection(features)[:, None, :]

        #2 Generate the target mask
        tgt_mask = torch.tril(torch.ones((T, T)))

        #3 Apply the docoder
        decoder_out = self.transformer(
          tgt=captions, memory=features, tgt_mask=tgt_mask) # (N, T, W)
