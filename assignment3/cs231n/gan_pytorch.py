...
def discriminator_loss(logits_real, logits_fake):
    ...
    ## Calculate BCE loss. The factor of 2 accounts for taking an average 
    ## over the true and false labels considering only true and only false
    ## labels when taking the average.
...
def build_dc_classifier(batch_size):
    ...
    model = nn.Sequential(
      ...
      nn.Conv2d(in_channels=32, out_channels=64, bias=True,stride=1, kernel_size=5),
      nn.LeakyReLU(negative_slope=0.01),
      nn.MaxPool2d(kernel_size=2, stride=2),
      nn.Flatten(),
