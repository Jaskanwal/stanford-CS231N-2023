...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        # Setting parameters for pool layer based on loss function.
        pool_height = 2
        pool_height = 2
        stride = 2
        H2 = int(1 + (H-pool_height)/stride)
        W2 = int(1 + (W-pool_height)/stride)
        self.params["W2"] = np.random.normal(
          loc=0, 
          scale=weight_scale,
          size=(H2*W2*num_filters, hidden_dim)
        )
        self.params["b2"] = np.zeros(hidden_dim)

        # Parameters for the third fully connected layer
    ...
    def loss(self, X, y=None):
        ...
        scores, cache_3 = affine_forward(out, W3, b3)
